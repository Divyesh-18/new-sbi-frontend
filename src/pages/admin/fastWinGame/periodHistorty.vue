            ...
            <div class="period_history_table ">
                ...
                <nav class="navbar pr-0 pl-0">
                    ...
                    <div class="periodhistory-search-box d-flex ml-auto" role="search">
                        <label>Search:</label>
                        <input class="form-control p-1" v-model="searchTerms" @input="search()" type="search"
                            ...
                            <tr>
                                ...
                                <th scope="col" class="font-weight-bold text-center add-table-data-in-center-width">
                                    <i class="bi bi-arrow-down-up mr-1 cursor-pointer "
                                        @click="sort('userTotleBateAmount')"></i>Total
                                    Bate Amount

                                </th>
                                <th scope="col" class="font-weight-bold text-center add-table-data-in-center-width">
                                    <i class="bi bi-arrow-down-up mr-1 cursor-pointer"
                                        @click="sort('TotalWinAmount')"></i>Total Win
                                    Amount
                                ...
                                <th scope="col" class="font-weight-bold text-center">
                                    ...
                                    <i class="bi bi-arrow-down-up mr-1 cursor-pointer"
                                        @click="sort('profitLoss')"></i>Profit/Loss

                                </th>
                                <th scope="col" class="font-weight-bold text-center">

                                    <i class="bi bi-arrow-down-up mr-1 cursor-pointer"
                                        @click="sort('created_at')"></i>Create Date
                        ...
                        <tbody v-if="data?.length > 0">
                            <tr v-for="(item, index) in data" :key="item._id">
                                ...
                                <td scope="col" class="text-center p-1 m-0">
                                    <router-link
                                        :to="`/dgklhtobgoijbrtoidfbgfgerjot/fast/win/user/period/history/${item?.gameid}`">
...
export default {
    ...
    methods: {
        async getPeriodIdData() {
            try {
                this.loader = true;
                const response = await axios.post("/admin/fast-win-period-history", {
        ...
        async period_bateing_history(periodid) {
            const response = await axios.post(`/admin/fast-win-period-id-wise-user-history`, {
                gameid: periodid,
            });
