  ...
  <div class="admin-layout">
    ...
    <div class="main-content-wrapper main-content-table Period_History">
      <div class="period_history_table">
        <h3 class="p-1 mr-0 add-bg-color-sub-header withdrawal-request-title">
          Withdrawal Request
        </h3>
        <nav class="navbar pr-0 pl-0">
          ...
          <div class="withdrawal-request-search-box d-flex ml-auto" style="padding-top: 5px !important;" role="search">
            <label>Search:</label>
        ...
        <div class="table-responsive table-responsive-sm">
          <table class="table withdrawal-request-table" v-if="loader === false">
            <thead>
              <tr>
            ...
            <tbody v-if="data.length > 0">
              <tr v-for="(item, index) in data" :key="item._id">
                ...
                <td class="d-flex">
                  <button class="btn-light ml-1 mr-1" v-if="item?.status !== 3"
                    v-on:click="adminAgreeWithdrawal(item?._id)">
                    Agree
                  </button>
                  <div v-if="message" class="alert alert-success">
                    {{ message }}
                  </div>
                  <button class="btn-success ml-1 mr-1" v-on:click="AdminAcceptWithdrawal(item?._id)">
                    Accept
                  </button>
                  <div v-if="message" class="alert alert-success">
                    {{ message }}
                  ...
                  <button class="btn-light ml-1 mr-1" v-on:click="AdminRejectWithdrawal(item._id)">
                    Reject
                  </button>
                  <div v-if="message" class="alert alert-success">
                    {{ message }}
...
// import AdminAcceptWithdrawalVue from './AdminAcceptWithdrawal.vue';
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        this.loader = true;
        const response = await axios.post(`admin/withdrawalpendding`, {
    ...
    async AdminAcceptWithdrawal(_id) {
      try {
        const response = await axios.post(`/admin/adminwithdrawalaccept`, {
          _id,
        });
        this.AdminAcceptWithdrawalData = response.data.data;
        this.fetchData();
        setTimeout(() => {
          this.tost = "Withdrawal Accept Successfully";
        }, 1000);
        setTimeout(() => {
          this.tost = "";
        }, 5000);
      } catch (error) {
        console.log("Admin accept withdrawal internel server error", error);
      }
    },
    async adminAgreeWithdrawal(id) {
      try {
        const response = await axios.post(`/admin/adminAgreeWithdrawal`, {
          id,
        });
        this.AdminAcceptWithdrawalData = response.data.data;
        this.fetchData();
        setTimeout(() => {
          this.tost = "Withdrawal Agree Successfully";
        }, 1000);
        setTimeout(() => {
          this.tost = "";
        }, 5000);
    ...
    async AdminRejectWithdrawal(_id) {
      try {
        const response = await axios.post(`/admin/adminwithdrawalreject`, {
          _id,
        });
        this.AdminRejectWithdrawaldata = response.data.data;
        this.fetchData();
        setTimeout(() => {
          this.tost = "Withdrawal Reject ";
        }, 1000);
        setTimeout(() => {
          this.tost = "";
        }, 5000);
